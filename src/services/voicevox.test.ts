...
describe('voicevox', () => {
  ...
  describe('createAudioQuery', () => {
    ...
    it('異なるスピーカーIDで呼び出せる', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        expect.stringContaining('speaker=3'),
        expect.any(Object)
      );
    });
    ...
    it('APIエラー時にエラーをスローする', async () => {
      ...
      await expect(createAudioQuery('テキスト', 0, mockBaseUrl)).rejects.toThrow(
        'audio_query failed: 400'
      );
    });
  });
  ...
  describe('synthesis', () => {
    it('音声を合成できる', async () => {
      ...
      expect(global.fetch).toHaveBeenCalledWith(
        `${mockBaseUrl}/synthesis?speaker=0`,
        {
          method: 'POST',
    ...
    it('APIエラー時にエラーをスローする', async () => {
      ...
      await expect(synthesis(mockQuery, 0, mockBaseUrl)).rejects.toThrow(
  ...
  describe('speak', () => {
    it('テキストから音声を生成できる（統合）', async () => {
      ...
      expect(global.fetch).toHaveBeenNthCalledWith(
        1,
        expect.stringContaining('/audio_query'),
        expect.any(Object)
      );
      expect(global.fetch).toHaveBeenNthCalledWith(
        2,
    ...
    it('audio_queryのエラーを伝播する', async () => {
      ...
      await expect(speak('テキスト', 0, mockBaseUrl)).rejects.toThrow(
        'audio_query failed: 400'
      );
    });
