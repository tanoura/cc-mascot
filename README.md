...
- **起動時にアップデートを確認する**: インターネット通信を行ってアップデートチェックをします。

## セッションフィルタリング（Claude Codeプラグイン）

Claude Codeを複数セッション並列で使用する場合、別途プラグインを用いることで特定のセッションのみを発話対象にフィルタリングできます。
...
また、設定画面からもフィルタの状態確認・解除が可能です。

> [!NOTE]
> プラグインは現時点ではmacOSのみ対応しています。
